...
export class CcClient {
  ...
  getUrl(getUrl) {
    ...
    const result = new URL(left + right);
  ...
  /**
   * @param {SimpleCommand<Api, ?, ?>} _command
   * @param {Partial<CcRequestParams>} requestParams
   * @param {Partial<CcRequestConfig>} [requestConfig]
   * @returns {Promise<CcRequest>}
   * @protected
   */
  async _prepareRequest(_command, requestParams, requestConfig) {
  ...
  async _handleResponse(response, command) {
    ...
    // handle http errors
    handleHttpErrors(response, command);
