 ...
 * @typedef {import('./endpoint.types.js').QueryParam} QueryParam
 * @typedef {import('./endpoint.types.js').PathParam} PathParam
 * @typedef {import('./endpoint.types.js').EndpointType} EndpointType
 * @typedef {import('./endpoint.types.js').EndpointResponse} EndpointResponse
 * @typedef {import('@scalar/openapi-types').OpenAPI.Document} Document
 * @typedef {import('@scalar/openapi-types').OpenAPIV3.OperationObject<{}>} OperationObject
 * @typedef {import('@scalar/openapi-types').OpenAPIV3.SchemaObject} SchemaObject
 * @typedef {import('@scalar/openapi-types').OpenAPIV3.ReferenceObject} ReferenceObject
 * @typedef {import('@scalar/openapi-parser').DereferenceResult} DereferenceResult
 */

/**
 * @param {Document} openapiObject the openapi object
  ...
  Object.entries(dereferenced.schema.paths).forEach(([p, methods]) => {
    ...
    if (methods.$ref) {
      ...
      Object.entries(methods).forEach((entry) => {
        ...
        const operationSpec = /** @type {OperationObject}*/ (dereferenced.specification.paths[p][method]);
...
/**
 *
 * @param {ReferenceObject|SchemaObject} dereferencedType
 * @param {ReferenceObject|SchemaObject} specificationType
