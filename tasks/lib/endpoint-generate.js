 ...
 */
function getBaseCommand(target, composite, autoOwner) {
  if (target === 'api') {
    if (composite) {
      if (autoOwner) {
        return {
          className: 'CcApiCompositeWithOwnerCommand',
      ...
      if (autoOwner) {
        return {
          className: 'CcApiSimpleWithOwnerCommand',
          classImport: '../../lib/cc-api-command.js',
        };
      } else {
        return {
...
function getClassContent(target, endpoint, namespace, className, composite, autoOwner) {
  const baseCommand = getBaseCommand(target, composite, autoOwner);
...
`;
  ...
  async compose(params, composer) {
    ...
    const requestParamsCall = hasPotentialRequestBody(endpoint)
      ? `${methodUtilFunctionName}(\`${endpoint.normalizedPath}\`, {})`
      : `${methodUtilFunctionName}(\`${endpoint.normalizedPath}\`)`;
...
interface ${inputInterface} extends ApplicationId {}
...
interface ${inputInterface} extends AddonId {}
...
export function generateCommand(target, namespace, commandClassName, composite, autoOwner, endpoint) {
  ...
  const commandOutputDir = path.join(SRC_DIR, `./clients/${target}/cmds/${kebabCase(namespace)}`);
