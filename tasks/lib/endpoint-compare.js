 ...
 * @return {OpenapiDiff}
 */
export function compareEndpoints(before, after) {
  // todo: handle ignored paths

  // new paths
  const addedEndpoints = Object.values(after).filter(({ id }) => before[id] == null);

  // deleted paths
  const deletedEndpoints = Object.values(before).filter(({ id }) => after[id] == null);

  // paths diff
  const modifiedEndpoints = Object.values(before)
    .filter(({ id }) => after[id] != null)
    .filter((path) => JSON.stringify(path) !== JSON.stringify(after[path.id]));
