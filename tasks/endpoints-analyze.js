/**
 * download openapis to .temp dir
 *   - openapi-clever-v2.json
 *   - openapi-clever-v4.json
 *   - openapi-clever-v4-ovd.json
 *   - auth backend
 *   - redis http
 *
 * generate a simplier version
 *  * map of <endpoint_id, endpoint>
 *    * endpoint_id = PATH + normalized path
 *    * endpoint = { method, path, parameters, requestBody, errors, operationId }
 *      * pathParameters: name, required, type
 *      * queryParameters: name, required, type
 *      * expected request headers (?? investigate this point)
 *      * requestBody: type
 *      * error: Map<code, type>
 *  * types can be 'BROKEN' when schema #ref are broken
 *  *
 *
 * filter with an ignore list of endpoint_id
 *
 * find diff with current openapi
 *  * new endpoints
 *  * removed endpoints
 *  * changed endpoints
 *
 * detect problems
 *  * use of snake case in
 *  * 'BROKEN' type
 *  * no operationId
 *  * multiple contentType response (?? investigate this point)
 *  * duplicate operationId
 *  * duplicate endpoint_id
 *  * not described path parameters
 */
...
/**
 * @typedef {import('./lib/endpoint.types.js').AnalyzedOpenapi} AnalyzedOpenapi
 * @typedef {import('./lib/endpoint.types.js').EndpointsSource} EndpointsSource
 * @typedef {import('./lib/endpoint.types.js').Endpoint}
 */
...
async function run() {
  ...
  for (let source of SOURCES) {
    ...
    if (!(await fs.pathExists(versionedFilePath))) {
      ...
      analyzedOpenapi.diff = {
        hasDiff: true,
        addedEndpoints: Object.values(endpoints),
      };
    } else {
      const previousVersion = await fs.readJson(versionedFilePath);
      const previousEndpoints = await parseEndpoints(previousVersion.openapi, previousVersion.source.pathPrefix);
      ...
      // compare
      analyzedOpenapi.diff = compareEndpoints(previousEndpoints, endpoints);
    }
  ...
  if (withDiffs.length > 0) {
    console.log(styleText(['bold', 'underline'], 'Generating report...'));
