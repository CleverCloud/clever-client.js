...
import type { LoadBalancerDnsConfig, ResolveDnsResult } from '../../../src/utils/domain-diag.js';
import { diagDomain } from '../../../src/utils/domain-diag.js';
...
describe('domain-diag#diagDomain()', () => {
  ...
  describe('Apex host', () => {
    ...
    it('All A records valid', () => {
      ...
      const domainDiagResults = diagDomain(domainInfo, resolveDnsResult, loadBalancerDnsConfig);
  ...
  describe('Host with subdomain', () => {
    ...
    it('Test-only host', () => {
      ...
      const domainDiagResults = diagDomain(domainInfo, undefined, undefined);
    ...
    it('should throw when DNS resolution and load balancer config are missing for a non-test domain', () => {
      const diagDomainCallback = () => diagDomain(domainInfo, undefined, undefined);
      expect(diagDomainCallback).toThrow(
        'Cannot diagnose a non-test domain without DNS resolution and load balancer config',
      );
    });
  });
});
