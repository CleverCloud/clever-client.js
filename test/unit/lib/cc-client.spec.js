...
describe('clever-client', () => {
  ...
  describe('simple command', () => {
    ...
    it('should call `auth.applyOnRequestParams()` when `command.isAuthRequired` returns true', async () => {
      const auth = new CcAuth();
      const spy = spyMethod(auth, 'applyOnRequestParams');
    ...
    it('should merge prepared request params from prepareRequestParams hook', async () => {
  ...
  describe('get url', () => {
    ...
    it('should construct the right url with auth', async () => {
      const gu = getUrl('/example');
      hanbi.stubMethod(gu, 'isAuthRequired').returns(true);
      const auth = new CcAuth();
      const spy = spyMethod(auth, 'applyOnUrl');
