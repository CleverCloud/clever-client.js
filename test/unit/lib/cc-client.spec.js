...
import { expectPromiseThrows } from '../../lib/expect-async.js';
import { mockTestHooks } from '../../lib/mock-api/support/mock-test-hooks.js';
import { spyMethod } from '../../lib/stub.js';
...
class SpiedClient extends CcClient {
  /** @type {CcClient<'test'>['_getAuth']} */
  _getAuth() {
    return super._getAuth();
  }
  ...
  async _handleResponse(response, command) {
    return super._handleResponse(response, command);
  }
}
...
describe('clever-client', () => {
  ...
  function createClient(config) {
    return new SpiedClient({ ...config, baseUrl: apiMockCtrl.mockClient.baseUrl });
  }
  ...
  describe('simple command', () => {
    it('should call `_transformCommandParams` method with right params', async () => {
      const spy = spyMethod(client, '_transformCommandParams');
    ...
    it('should call `_getCommandRequestParams` method with right params', async () => {
      const spy = spyMethod(client, '_getCommandRequestParams');
      const command = simpleCommand(get('/path/subPath'));
    ...
    it('should call `_prepareRequest` method with right params', async () => {
      const spy = spyMethod(client, '_prepareRequest');
      const command = simpleCommand(get('/path/subPath'));
      const requestConfig = {};
      ...
      expect(spy.firstCall.args[1].url).to.equal('/path/subPath');
      expect(spy.firstCall.args[2]).to.equal(requestConfig);
    });
    ...
    it('should call `command.toRequestParams` method with the transformed params', async () => {
      ...
      const command = simpleCommand(get('/path/subPath'));
      const spy = spyMethod(command, 'toRequestParams');
    ...
    it('should not call `_getAuth` method when `command.isAuthRequired` returns false', async () => {
      const spy = spyMethod(client, '_getAuth');
      const command = simpleCommand(get('/path/subPath'));
      hanbi.stubMethod(command, 'isAuthRequired').returns(false);
    ...
    it('should call `_getAuth` method when `command.isAuthRequired` returns true', async () => {
      const spy = spyMethod(client, '_getAuth');
      const command = simpleCommand(get('/path/subPath'));
      hanbi.stubMethod(command, 'isAuthRequired').returns(true);
    ...
    it('should call function returned by `_getAuth` method when `command.isAuthRequired` returns true', async () => {
      const spy = hanbi.stub((o) => o);
      spy.passThrough();
      hanbi.stubMethod(client, '_getAuth').returns(spy.handler);
      const command = simpleCommand(get('/path/subPath'));
      hanbi.stubMethod(command, 'isAuthRequired').returns(true);
    ...
    it('should call prepareRequestParams hook function', async () => {
      const spy = hanbi.stub((o) => o);
      spy.passThrough();
      const client = createClient({
        hooks: {
          prepareRequestParams: spy.handler,
        },
      });
      const command = simpleCommand(get('/path/subPath'));
    ...
    it('should merge prepared request params from prepareRequestParams hook and auth function', async () => {
      const prepareRequestParams = /** @param {Partial<CcRequestParams>} request */ (request) => ({
        ...request,
        queryParams: new QueryParams(request.queryParams?.toObject()).set('hook', 'hook'),
      });
      const auth = /** @param {Partial<CcRequestParams>} request */ (request) => ({
        ...request,
        queryParams: new QueryParams(request.queryParams?.toObject()).set('auth', 'auth'),
      });

      const client = createClient({ hooks: { prepareRequestParams } });
      hanbi.stubMethod(client, '_getAuth').returns(auth);
      const spy = spyMethod(client, '_prepareRequest');
      const command = simpleCommand(get('/path/subPath'));
      hanbi.stubMethod(command, 'isAuthRequired').returns(true);
      ...
      expect(result.queryParams.get('auth')).to.equal('auth');
    });
    ...
    it('should call `_handleResponse` with right parameters', async () => {
      const spy = spyMethod(client, '_handleResponse');
      const command = simpleCommand(get('/path/subPath'));
      ...
      expect(spy.firstCall.args[0].request.method).to.equal('GET');
      expect(spy.firstCall.args[0].request.url).to.equal(`${apiMockCtrl.mockClient.baseUrl}/path/subPath`);
      expect(spy.firstCall.args[1]).to.equal(command);
    });
    ...
    it('should call `command.transformCommandOutput` with right parameters', async () => {
      const command = simpleCommand(get('/path/subPath'));
      const spy = spyMethod(command, 'transformCommandOutput');
    ...
    it('should throw `CcHttpError` when API returns error status', async () => {
      ...
      await apiMockCtrl.mock().when('GET', '/path/subPath').return(500, 'A server error occurred');
  ...
  describe('composite command', () => {
    it('should call `_compose` method with right params', async () => {
      const spy = spyMethod(client, '_compose');
    ...
    it('should call `command.compose` method with right params', async () => {
      ...
      hanbi.stubMethod(client, '_transformCommandParams').returns(Promise.resolve(transformedParams));
      const command = compositeCommand('command result');
      const spy = spyMethod(command, 'compose');
    ...
    it('composer should merge request config with initial request config', async () => {
      const command = new (class MyCommand extends TestCompositeCommand {
        ...
        async compose(_params, composer) {
          composer.send(simpleCommand(get('/path/subPath')), { cors: true, timeout: 1000 });
          return 'result';
        }
      })();
      const requestConfig = { cors: false, cache: true };
      const spy = spyMethod(client, 'send');
      await apiMockCtrl.mock().when('GET', '/path/subPath').return(200, 'body');
