...
describe('request', () => {
  ...
  describe('sendRequest', () => {
    it('should successfully make a GET request', async () => {
      ...
      await apiMockCtrl
        ...
        .verify((calls) => {
          ...
          expect(calls.first.response.status).to.equal(200);
          expect(calls.first.response.body).to.deep.equal(responseBody);
        });
    });
    ...
    it('should successfully make a POST request with JSON body', async () => {
      ...
      await apiMockCtrl
        ...
        .verify((calls) => {
          ...
          expect(calls.first.response.status).to.equal(201);
          expect(calls.first.response.body).to.deep.equal(responseBody);
        });
    });
    ...
    it('should successfully make a DELETE request', async () => {
      await apiMockCtrl
        ...
        .verify((calls) => {
          ...
          expect(calls.first.path).to.equal('/api/test');
          expect(calls.first.response.status).to.equal(204);
        });
    });
    ...
    it('should handle unknown errors', async () => {
      ...
      await expectPromiseThrows(
        ...
        (error) => {
          expect(error).to.be.instanceOf(CcRequestError);
          expect(error.code).to.equal('UNKNOWN_ERROR');
          expect(error.message).to.include('An unknown error occurred');
        },
      );
    });

    it('should set CORS mode when specified', async () => {
      await apiMockCtrl.mock().when('GET', '/api/test').return(201);
      const spy = spyMethod(globalThis, 'fetch');
  ...
  describe('cache', () => {
    it('should use cached response', async () => {
      const responseBody = { data: 'test response' };
      await apiMockCtrl.mock().when('GET', '/api/test').return(200, { data: 'test response' });
      await sendRequest({
        method: 'GET',
        url: `/api/test`,
        headers: new HeadersBuilder().acceptJson().build(),
    ...
    it('should reload cached response', async () => {
      ...
      await apiMockCtrl.mock().when('GET', '/api/test').return(200, newResponseBody);
      const spy = spyMethod(globalThis, 'fetch');
  ...
  describe('timeout', () => {
    it('should timeout', async function () {
      ...
      await apiMockCtrl.mock().when('GET', '/api/test').return(200);
      const stub = hanbi.stubMethod(globalThis, 'fetch');
      stub.callsFake(() => {
        return new Promise((resolve) => {
          // @ts-ignore
          setTimeout(() => resolve({ headers: new Headers(), text: () => Promise.resolve('') }), 20);
        });
      });
