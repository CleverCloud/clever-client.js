...
describe.skip('pulsar commands', function () {
  ...
  it('should get pulsar info', async () => {
    ...
    expect(response.cluster.isAvailable).toBeTypeOf('boolean');
    expect(response.cluster.zone).toBeTypeOf('string');
    expect(response.cluster.isColdStorageSupported).toBeTypeOf('boolean');
    ...
    expect(response.isColdStorageMustBeProvided).toBeTypeOf('boolean');
  });
});
