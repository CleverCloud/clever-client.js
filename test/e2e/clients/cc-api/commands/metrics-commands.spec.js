...
import { e2eSupport, STATIC_ADDON_ID } from '../e2e-support.js';
...
describe('metrics commands', function () {
  ...
  it('should get addon memory metrics', async () => {
    const response = await support.client.send(new GetMetricsCommand({ addonId: STATIC_ADDON_ID, metrics: ['mem'] }));
  ...
  it('should get addon metrics', async () => {
    const response = await support.client.send(new GetMetricsCommand({ addonId: STATIC_ADDON_ID }));
