...
describe('domains', () => {
  describe('parseDomain()', () => {
    ...
    it('should throw DomainParseError for invalid wildcard', () => {
      ...
      expect(parseDomainCallback).to.throw(DomainParseError, 'Invalid wildcard format');
    });

    it('should throw DomainParseError for empty domain', () => {
      const parseDomainCallback = () => parseDomain('');
      expect(parseDomainCallback).to.throw(DomainParseError, 'Empty domain value');
    });

    it('should throw DomainParseError for invalid format', () => {
      const parseDomainCallback = () => parseDomain('].com');
      expect(parseDomainCallback).to.throw(DomainParseError, 'Invalid domain format');
    });
  });
  ...
  describe('isPrivateLoadBalancerCname()', () => {
    it('should return true for private load balancer CNAME', () => {
      const isPrivateLB = isPrivateLoadBalancerCname('app-123-customers.services.clever-cloud.com.');
      expect(isPrivateLB).to.be.equal(true);
    });

    it('should return false for non-private load balancer CNAME', () => {
      const isPrivateLB = isPrivateLoadBalancerCname('example.com.');
      expect(isPrivateLB).to.be.equal(false);
    });
  });
