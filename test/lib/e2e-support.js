...
import { deepEqualInAnyOrder } from './deep-equal-in-any-order/deep-equal-in-any-order.js';
import { e2eTestUsers } from './e2e-test-users.js';

/**
 * @typedef User
 * @type {'test-user-without-github'|'test-user-with-github'}
 */
/**
 * @typedef Auth
...
export const STATIC_ADDON_ID = 'addon_2066c7dd-5891-420a-ae3c-2334405c3bf1';
export const STATIC_INVOICE_ID = ''; // todo: set the right invoice id when ready
export const STATIC_ORGANISATION_ID = 'orga_3659ccc6-1b06-4393-83d6-52dc3d72416d';

/**
 * @param {{user?: User, auth?: Auth, debug?: boolean }} [config]
 */
export function e2eSupport(config) {
  const conf = {
    ...{ user: DEFAULT_USER, auth: DEFAULT_AUTH, debug: false },
    ...(config ?? {}),
  };

  const client = createCcApiClient(conf.user, conf.auth, conf.debug);
  /** @type {string} */
  let organisationId;
  /** @type {string} */
  let userId;
  ...
  return {
     ...
     */
    getClient({ user, auth }) {
      return createCcApiClient(user ?? DEFAULT_USER, auth ?? DEFAULT_AUTH, conf.debug);
    },
    get client() {
      return client;
    },
...
function createCcApiClient(user, auth, debug) {
  ...
  if (USE_LOCAL_AUTH_BACKEND && auth === 'oauth-v1') {
    throw new Error('oauth-v1 is not supported with local auth backend');
  }
...
function getCcApiAuth(user, auth) {
  ...
  const testUser = e2eTestUsers.get(user);
  if (!testUser) {
  ...
  if (auth === 'oauth-v1') {
    return {
      type: 'oauth-v1',
      oauthTokens: testUser.oauthTokens,
    };
  }
